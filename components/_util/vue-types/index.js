...
const VuePropTypes = {
    ...
    shape(obj) {
        ...
        const requireKeys = keys.filter(key => obj[key] && obj[key].required === true)
        const type = toType('shape', {
            ...
            validator(value) {
                ...
                if (requireKeys.length > 0 && requireKeys.some(req => valueKeys.indexOf(req) === -1)) {
                    warn(
                        `shape - at least one of required properties "${requireKeys.join(
                            '", "',
                        )}" is not present`,
                    )
                    return false
